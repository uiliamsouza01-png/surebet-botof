...
# Nomes amigáveis para exibir nos alertas (cai no valor cru se não mapeado)
SPORT_DISPLAY_NAMES = {
...
def sport_display_name(sport_key: str) -> str:
    """Nome amigável do esporte para exibir no alerta, com fallback pro valor cru."""
    return SPORT_DISPLAY_NAMES.get(sport_key, sport_key.replace("_", " ").title())


def format_alert(sport_key: str, event: dict, arb: dict) -> str:
    """
    Monta o texto do alerta. Texto puro, sem parse_mode — não precisa
    escapar Markdown/HTML porque o bot manda como texto simples.
    """
    ...
    lines = [
        ...
        f"🏆 {sport_display_name(sport_key)}",
        f"⚔️ {home} x {away}",
        f"🕒 Início: {commence}",
        "",
...
async def send_alert(app: Application, chat_id: str, message: str):
    """Envia o alerta e nunca deixa uma falha de rede/Telegram derrubar o loop."""
    try:
        await app.bot.send_message(chat_id=chat_id, text=message)
    except TelegramError:
        logger.exception("Falha ao enviar alerta pelo Telegram")
    ...
    for sport in SPORTS:
        ...
        for event in events:
            ...
            if arb and arb["profit_percent"] >= MIN_PROFIT_PERCENT:
                ...
                await send_alert(app, TELEGRAM_CHAT_ID, message)
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    sports_display = ", ".join(sport_display_name(s.strip()) for s in SPORTS if s.strip())
    await update.message.reply_text(
        "📊 Status do scanner\n"
        f"Esportes: {sports_display}\n"
        f"Regiões: {REGIONS}\n"
